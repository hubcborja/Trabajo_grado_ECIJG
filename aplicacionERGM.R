...
network.edgecount(network_comtrade)
...
# Prueba t de Welch para diferencia de medias (Bilateral)
...
# Prueba de robustez no paramétrica (Mann-Whitney / Wilcoxon)
...
# 4. Cruzar el esqueleto masivo con base histórica
...
hacer_matriz <- function(df, variable, relleno = 0) {
  m <- df %>%
    select(iso3_o, iso3_d, valor = all_of(variable)) %>%
    pivot_wider(names_from = iso3_d, values_from = valor) %>%
    arrange(iso3_o)
  filas <- m$iso3_o
  m <- as.matrix(m[, -1])
  rownames(m) <- filas
  m <- m[paises_217, paises_217]
  m[is.na(m) | is.infinite(m)] <- relleno
  diag(m) <- 0
  m
}

lista_redes <- list()
cov_CDT  <- list(); cov_fta <- list(); cov_PIB <- list()
...
for (i in seq_along(anios_panel)) {
  ...
  mat_lazo <- hacer_matriz(base_temp, "lazo_comercial")
  lista_redes[[i]] <- as.network(mat_lazo, directed = TRUE,
                                 matrix.type = "adjacency", loops = FALSE)
  
  cov_CDT[[i]] <- hacer_matriz(base_temp, "CDT_vigente")
  cov_fta[[i]] <- hacer_matriz(base_temp, "fta_wto")
  cov_PIB[[i]] <- hacer_matriz(base_temp, "ln_suma_PIB_capita",
                               relleno = median(base_temp$ln_suma_PIB_capita, na.rm = TRUE))
  
  cat("Año", anio_actual, "procesado. Densidad:",
      round(network.density(lista_redes[[i]]), 3), "\n")
}

# --- Covariables constantes en el tiempo: UNA sola matriz (no lista) ---
base_2024 <- baseERGM_masiva %>% filter(year == 2024)
cov_dist <- hacer_matriz(base_2024, "ln_dist",
                         relleno = median(base_2024$ln_dist, na.rm = TRUE))
cov_lang <- hacer_matriz(base_2024, "comlang_off")

# Chequeo de sanidad
sapply(lista_redes, network.density)

# Modelo A — base (correr primero como prueba de humo)
f_base <- lista_redes ~ edges + mutual +
  edgecov(cov_CDT) + edgecov(cov_fta) +
  edgecov(cov_dist) + edgecov(cov_PIB) + edgecov(cov_lang) +
  memory(type = "stability", lag = 1)

fit_prueba <- btergm(f_base, R = 50,
                     parallel = "snow", ncpus = nucleos,
                     usefastglm = TRUE, verbose = TRUE)
summary(fit_prueba)

# Modelo B — completo (agrega dependencia triádica y reciprocidad rezagada)
f_full <- lista_redes ~ edges + mutual +
...
# Corrida final (solo cuando la especificación esté cerrada):
fit_final <- btergm(f_full, R = 500,
                    parallel = "snow", ncpus = nucleos,
                    usefastglm = TRUE, verbose = TRUE)
summary(fit_final)

gof_final <- gof(fit_final, nsim = 25,
                 statistics = c(esp, geodesic, deg, rocpr))
plot(gof_final)
