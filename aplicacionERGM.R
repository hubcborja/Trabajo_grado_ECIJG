...
network_comtrade <- as.network(baseERGM24, directed=TRUE, matrix.type = "edgelist", 
...
plot(network_comtrade, edge.lwd = baseERGM24$ln_peso_comercio_total, main = "peso_comercio_total")

baseERGM24$ln_peso_comercio_total <- round(baseERGM24$ln_peso_comercio_total)
...
ergm.fit_base <- ergm(formula = ergm_base, response='ln_peso_comercio_total', 
                      reference = ~Poisson,
...
ergm_todas <- network_comtrade ~ edges + sum +
  mutual(form="min") + 
  transitiveweights("min", "max", "min") +
  edgecov(network_comtrade, "CDT_vigente") + 
  edgecov(network_comtrade, "fta_wto") +    
  edgecov(network_comtrade, "ln_dist") +
  nodecov("ln_suma_PIB_capita") +
  edgecov(network_comtrade, "comlang_off") 

ergm.fit.todas <- ergm(formula = ergm_todas, response='ln_peso_comercio_total', reference = ~Poisson)
